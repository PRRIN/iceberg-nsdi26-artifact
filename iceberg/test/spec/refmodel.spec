...
fn is_prefix(rname: List<Int>, qname: List<Int>) -> Bool {
    ...
    let qlen = len(qname);
    rlen <=s qlen
    && list_eq::<Int>(rname, qname[..rlen])
}

fn is_wild_card_domain (rr: DnsRecord) -> Bool {
    let rname = rr.rname;
    let last = len(rname) - 1;
    rname[last] == wild_card_code() && rr.rtype != DnsRRType.EMPTY
}

fn is_wild_card_match(rr: DnsRecord, qname: List<Int>) -> Bool {
    let rname = rr.rname;
    let idx = len(rname) - 1;
    len(rname) <=s len(qname)
    && is_wild_card_domain(rr)
    && qname[idx] != wild_card_code()
    && list_eq::<Int>(rname[..idx], qname[..idx])
}

fn is_relevant_domain(rr: DnsRecord, qname: List<Int>) -> Bool {
    let rname = rr.rname;
    if is_wild_card_domain(rr) {
        list_eq::<Int>(rname, qname) || is_wild_card_match(rr, qname)
    } else {
        is_prefix(rname, qname)
    }
}

fn is_ns_match_record(rrset: DnsRecord, qname: List<Int>, zone_name: List<Int>) -> Bool {
    rrset.rtype == DnsRRType.NS
    && !list_eq::<Int>(rrset.rname, zone_name)
...
fn filter_max_matches(rrs: List<DnsRecord>, qname: List<Int>) -> List<DnsRecord> {
    let matches = map::<DnsRecord, DnsRecordMatch>(
        rrs, 
        |rr| {
            DnsRecordMatch {
                record: rr,
                len: list_cmp::<Int>(rr.rname, qname),
            }
        }
    );
...
fn delegation(ns_records: List<DnsRecord>, all_records: List<DnsRecord>) -> List<DnsRecord> {
    ...
    let glue_records = filter::<DnsRecord>(
        ...
        |rr| {
            any::<DnsRecord>(
                ns_records, 
                |ns_rr| { list_eq::<Int>(ns_rr.rdata, rr.rname) } 
            )
        }
    );
    glue_records
...
fn rrlookup(zone: DnsZone, query: DnsQuery) -> DnsResponse {
    ...
    if len(relevant_rrs) == 0 {
        ...
        if any::<DnsRecord>(relevant_rrs, |x| {list_eq::<Int>(x.rname, query.qname)}) {
